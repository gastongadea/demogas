...
/** Actualiza cupo_maximo desde la planilla para tutores ya existentes en Neon. */
async function syncCuposFromSheet() {
  ...
  for (const tutor of tutors) {
    const existingId = await findExistingTutorId(pool, tutor);
    if (!existingId) continue;

    const res = await pool.query(
      `UPDATE tutores
       SET cupo_maximo = $1, updated_at = now()
       WHERE id = $2 AND cupo_maximo IS DISTINCT FROM $1
       RETURNING id`,
      [tutor.cupo_maximo, existingId]
    );
    if (res.rows.length > 0) updated += 1;
  }

  return { cuposUpdated: updated };
}
